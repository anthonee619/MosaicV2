...
export default class JimpImage implements Image {
  ...
  public save(imageName?: string): Promise<string> {
    return new Promise<any>((resolve, reject) => {
      ...
      this.image.quality(50);
      this.image.write(outputImageName, (err: any, _: any) => {
        if (err) {
          reject(err);
        }
        else {
  ...
  public getAverageColor(x_start: number = 0, y_start: number = 0,
    ...
    return new Promise<RGB>((resolve, reject) => {
      let r = 0, g = 0, b = 0;
      let i = 0;
      this.image.scan(x_start, y_start, width, height, function(x: any, y: any, idx: any) {
